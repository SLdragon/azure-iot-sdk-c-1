...
void e2e_uploadtoblob_test_multi_threaded(IOTHUB_CLIENT_TRANSPORT_PROVIDER protocol)
{
    IOTHUB_CLIENT_RESULT result;
    IOTHUB_PROVISIONED_DEVICE* deviceToUse = IoTHubAccount_GetSASDevice(g_iothubAcctInfo);
    ASSERT_IS_NOT_NULL(deviceToUse);
    ...
    for (int i = 0; i < TEST_MAX_SIMULTANEOUS_UPLOADS; i++)
    {
        ...
        result = IoTHubClient_UploadToBlobAsync(iotHubClientHandle, UPLOADTOBLOB_E2E_TEST_DESTINATION_FILE, UPLOADTOBLOB_E2E_TEST_DATA, strlen((const char*)UPLOADTOBLOB_E2E_TEST_DATA), uploadToBlobCallback, &uploadToBlobStatus[i]);
        ASSERT_ARE_EQUAL_WITH_MSG(IOTHUB_CLIENT_RESULT, IOTHUB_CLIENT_OK, result, "Could not IoTHubClient_UploadToBlobAsync");
    }
    ...
    for (int i = 0; i < TEST_MAX_SIMULTANEOUS_UPLOADS; i++)
    {
        printf("waiting for context for(%p)\n", &uploadToBlobStatus[i]);
...
{
    e2e_uploadtoblob_test_multi_threaded(MQTT_Protocol);
}
